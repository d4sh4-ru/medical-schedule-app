...
import { fetchScheduleDetails, saveSchedule } from '../../services/planService';
...
export default function ScheduleFormScreen() {
  ...
  useEffect(() => {
    if (isEditing) {
      const loadSchedule = async () => {
        try {
          const data = await fetchScheduleDetails(scheduleId, navigation);
  ...
  const handleDateRangeSelect = (day) => {
    ...
    if (!dateRange.startDate || (dateRange.startDate && dateRange.endDate)) {
      ...
      setMarkedDates({
        [day.dateString]: { startingDay: true, color: '#007AFF', textColor: '#fff' },
      });
  ...
  const handleTimeChange = (event, selectedTime) => {
    console.log('handleTimeChange event:', event, 'selectedTime:', selectedTime);
  ...
  return (
    <SafeAreaView style={styles.scheduleFormScreen.container}>
      ...
      />
      <Text style={styles.scheduleFormScreen.label}>Дозировка одной таблетки (мг)</Text>
      <CustomPicker
        value={tabletDosage}
            ...
            <View style={styles.scheduleFormScreen.modalContainer}>
              <View style={styles.scheduleFormScreen.modalContent}>
                <DateTimePicker
                  ...
                  textColor="black"
                />
                <TouchableOpacity style={styles.common.button} onPress={handleTimeConfirm}>

...
export default function ScheduleScreen() {
  ...
  const openModal = (notification) => {
    ...
    fetchScheduleDetails(notification.schedule_id);
      ...
      [
        { text: 'Отмена', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await fetchWithAuth(
                `http://cloud-ru-test.netbird.cloud:8080/api/schedule/${id}`,
                { method: 'DELETE' },
                navigation
              );
            ...
            {scheduleDetails ? (
              <>
                ...
                <Text style={localStyles.modalText}>
                  Дозировка: {scheduleDetails.administrationMethod.singleDosageTablets} табл. x{' '}
                  {scheduleDetails.administrationMethod.singleDosage}
                </Text>
                <View style={{ marginBottom: 8 }}>
                  <Text style={localStyles.modalText}>Время приёма:</Text>
                  <FlatList
                    data={scheduleDetails.administrationMethod.administrationTimes.split(',')}
                ...
                <View style={localStyles.modalButtonContainer}>
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      navigation.navigate('ScheduleForm', {
                        scheduleId: selectedNotification?.schedule_id,
                      });
                    }}
                  >
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => handleDelete(selectedNotification?.schedule_id)}
                  >
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                      <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                        <Path
